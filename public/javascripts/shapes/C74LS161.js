...
C74LS161 = C74LS161.extend({
    ...
    calculate:function()
    {
        ...
        var nr = this.getInputPort("Port_nR");
        var nld = this.getInputPort("Port_nLD");
        var ct = this.getInputPort("Port_T");
        var p = this.getInputPort("Port_P");
        var d0 = this.getInputPort("Port_A");
        var d1 = this.getInputPort("Port_B");
        var d2 = this.getInputPort("Port_C");
        var d3 = this.getInputPort("Port_D");
        ...
        var co = this.getOutputPort("Port_Co");
        
        
        if(nr.getValue() === false)    this.counter = 0;
        else
        {
            if(nld.getValue() === false)
            {
                this.counter = Number(d0.getValue()) + 2 * Number(d1.getValue()) + 4 * Number(d2.getValue()) + 8 * Number(d3.getValue());
            }
            else
            {
                if (rising === true) {
                    if(ct.getValue() === true && p.getValue() === true) this.counter = (this.counter + 1) % 16;
