...
function drawCircuit_simplified(graph) {
    components = [];
    newEditSignalList = [];
    crossIds = {};

    for (let oldIn in editSignalList){
        let flag = false;
        for (let id in graph){
            if (graph[id].hash_id == editSignalList[oldIn].hash_id){
                flag = true;
                break;
            }
        }
        if (flag){
            crossIds[editSignalList[oldIn].hash_id] = oldIn;
        }
    }
    ...
    for (let id in graph) {
        ...
        case 'input':
            newChip = {};
            newChip.name = 'input';
            newChip.tag = graph[id].tag;
            //newChip.tagText.text = comp.tag;
            newChip.tagText = {};
            newChip.tagText.text = graph[id].tag;

            newIn = {ref: newChip};
            newIn.id = null;
            newIn.hash_id = graph[id].hash_id;
            if (crossIds[graph[id].hash_id] !== undefined) {
                newIn.list = editSignalList[crossIds[graph[id].hash_id]].list;
                newIn.list.pop();
                console.log(newIn.list);
            } else {
                newIn.list = [-1, 0];
            }

            newEditSignalList.push(newIn);
            break;
    ...
    console.log(editSignalList);
    editSignalList = newEditSignalList;

    //by zfw: edit the height to contain all siganals
    $(".wave-box").height(Math.max(300,27*editSignalList.length+141));
...
function saveStimFile() {
    var stimJson = {};
    stimJson.length = editSignalList.length;
    stimJson.data = editSignalList;
    return JSON.stringify(stimJson);
}

function loadStimFile(graph) {
    console.log(graph);
    if (graph.data !== undefined){
        editSignalList = graph.data;
    } else {
        for (let oldIn in editSignalList){
            editSignalList[oldIn].list = [-1,0];
        }
    }
...
function moveV4(event, g, context) {
    ...
    if (context.dragEndX + 20 > (g.size().width - divWidth - 2)){
        $(".dygraph-legend").css({"left":"80px"});
    } else {
        $(".dygraph-legend").css({"left":g.size().width - divWidth - 2+"px"});
    }
...
function createGraph(){
    ...
    for(k in editSignalList)
    {
        ...
        var HTMLTag = "<div style='width:50px;white-space:nowrap;text-overflow:ellipsis;overflow:hidden;' title='"+editSignalList[k].ref.tag+"'>"+editSignalList[k].ref.tag+"</div>";
...
function reloadJili(projectId,projectType) {
    // alert(editSignalList);
    //console.log("/file/reloadJili");
    var inputSignalList = [];
