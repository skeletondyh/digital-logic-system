...
chipPath.set("74LS14", "./chipEntities/Chip74LS14");
chipPath.set("74LS20", "./chipEntities/Chip74LS20");
chipPath.set("74LS27", "./chipEntities/Chip74LS27");
chipPath.set("BCDto7Seg", "./chipEntities/ChipBCDto7Seg");
/**
 *
 * @param savePath
...
function VHDLCodeGenerator(savePath, connectionInfo, topEntityName, portInfo,callback) {
    ...
    for(var chipId in connectionInfo)
    {
        ...
        if(chipName == "input"){
            console.log("info.tag: ",info.tag);
            //info.tag = "input" + chipId; // todo 这个需要使用前端传回的数据
            var curChip = new input(chipId, info.tag);
            ...
            //info.tag = "output" + chipId; // todo 这个需要使用前端传回的数据
            var curChip = new output(chipId, info.tag);
            curChip.pinMap.set("0", info.tag);
            idToEntity.set(chipId, curChip);
            ...
            //info.tag = "vcc" + chipId; // todo 这个需要使用前端传回的数据
            var curChip = new vcc(chipId, info.tag);
            curChip.pinMap.set("0", "vcc_" + chipId + "_0");
            idToEntity.set(chipId, curChip);
            idToVcc.set(chipId, curChip);
        }
        else if(chipName == "GND"){
            //info.tag = "gnd" + chipId; // todo 这个需要使用前端传回的数据
            var curChip = new gnd(chipId, info.tag);
            curChip.pinMap.set("0", "gnd_" + chipId + "_0");
            idToEntity.set(chipId, curChip);
            idToGnd.set(chipId, curChip);
        }
        else{
            info.tag = "chips" + chipId; // todo 这个需要使用前端传回的数据
